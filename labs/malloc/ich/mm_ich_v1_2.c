...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    size_t pre_alloc = GET_ALLOC(FTRP(PREV_BLKP(ptr)));
    size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(ptr)));
    size_t preSize = GET_SIZE(FTRP(PREV_BLKP(ptr)));
    size_t nextSize = GET_SIZE(HDRP(NEXT_BLKP(ptr)));
    // if(preSize + nextSize + oldSize >= size && (!pre_alloc && !next_alloc)) {
    //     newptr = PREV_BLKP(ptr);
    //     copySize = size;
    //     if(oldSize < copySize) copySize = oldSize;
    //     memcpy(newptr, ptr, copySize - WSIZE);
    //     size_t asize = preSize + nextSize + oldSize - size;
    //     if(asize < 2*DSIZE) {
    ...
    //         PUT(HDRP(NEXT_BLKP(newptr)), PACK(asize, 0));
    //         PUT(FTRP(NEXT_BLKP(newptr)), PACK(asize, 0));
    //     }
    //     return newptr;
    // }
    // else if(pre_alloc && preSize + oldSize - DSIZE >= size) {
    //     newptr = PREV_BLKP(ptr);
    //     copySize = size;
    //     if(oldSize < copySize) copySize = oldSize;
    //     memcpy(newptr, ptr, copySize - WSIZE);
    //     size_t asize = preSize + oldSize - size;
    //     if(asize < 2*DSIZE) {
    //         PUT(HDRP(newptr), PACK(size, 1));
    //         PUT(FTRP(newptr), PACK(size, 1));
    //     }
    ...
    //     return newptr;
    // }
    // else if(next_alloc && nextSize + oldSize - DSIZE >= size) {
    //     newptr = ptr;
    //     copySize = size;
    //     if(oldSize < copySize) copySize = oldSize;
    //     memcpy(newptr, ptr, copySize - WSIZE);
    //     size_t asize = nextSize + oldSize - size;
    //     if( asize < 2*DSIZE) {
    //         PUT(HDRP(newptr), PACK(size, 1));
    //         PUT(FTRP(newptr), PACK(size, 1));
    //     }
